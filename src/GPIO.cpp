...
//	goto cleanup;
	...
	if(!_chip) {
		ELOG_MESSAGE("Failed open GPIO chip(\"%s\") : %s \n",path.c_str(),strerror(errno));
		error = errno;
		goto cleanup;
	}
	...
	if(error) {
		ELOG_MESSAGE("Failed get GPIO lines: %s \n",strerror(errno));
		goto cleanup;
	}
	...
	if(error) {
		ELOG_MESSAGE("Failed request GPIO lines: %s \n",strerror(errno));
	goto cleanup;
	}
...
bool GPIO::setRelays(gpioStates_t states){
	...
	if(error) {
		ELOG_MESSAGE("Failed get line values GPIO lines: %s \n",strerror(errno));
		return false;
	}
	...
	if(error) {
		ELOG_MESSAGE("Failed get set values GPIO lines: %s \n",strerror(errno));
		return false;
	}
